...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  function thresholdMs(p){
    return p.idleDays*86400000+p.idleHours*3600000+
           p.idleMinutes*60000+p.idleSeconds*1000;
  }
  ...
  $('save').addEventListener('click',()=>{
    ...
    chrome.storage.sync.set(prefs,()=>{
      ...
      chrome.tabs.query({},tabs=>{
        for(const t of tabs){
          if(prefs.blacklist.some(p=>t.url.includes(p))){
  ...
  async function updateIdleList(){
    ...
    chrome.tabs.query({},tabs=>{
      const list=tabs
        ...
        .filter(t=>!prefs.whitelist.some(p=>t.url.includes(p)))
        .map(t=>{
